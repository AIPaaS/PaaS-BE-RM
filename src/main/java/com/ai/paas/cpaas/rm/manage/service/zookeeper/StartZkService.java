...
public class StartZkService implements Tasklet {
  private static Logger logger = Logger.getLogger(StartZkService.class);
  ...
  public RepeatStatus execute(StepContribution contribution, ChunkContext chunkContext)
    ...
    try {
      result = TaskUtil.executeFile("StartZkServiceStep", shellContext.toString(), useAgent, aid);
    } catch (Exception e) {
      logger.error("start zookeeper service:", e);
      result = e.toString();
      ...
      int taskId =
          TaskUtil.insertResJobDetail(start, openParam.getClusterId(), shellContext.toString(),
              TaskUtil.getTypeId("startZkServiceStep"), status);
